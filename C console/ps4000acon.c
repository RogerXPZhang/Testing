...
PICO_STATUS ChangePowerSource(int16_t handle, PICO_STATUS status)
{
	...
	switch (status)
	{
		...
		case PICO_USB3_0_DEVICE_NON_USB3_0_PORT:			// User must acknowledge they want to power via USB
				//printf("\nThis device has a requirement to be connected via:");
				//printf("\n    USB 3.0 cable to USB 3.0 port");
				//printf("\nor");
				//printf("\n    USB 2.0 dual-headed cable to USB 2.0 port");
				printf("\nPower OK\n");
...
void BlockDataHandler(UNIT * unit, int8_t * text, int32_t offset)
{
	...
	*		 the most suitable time units */
	while (ps4000aGetTimebase(unit->handle, timebase, sampleCount, &timeInterval, &maxSamples, 0))
	{
		timebase++;
	}

	printf("\nTimebase: %u  SampleInterval: %dnS\n", timebase, timeInterval);
	...
	if(g_ready)
	{
		...
		else
		{
			...
			if (fp != NULL)
			{
				...
				{
					fprintf(fp, "%5ld ", g_times[0] + (uint64_t)(i * timeInterval));
...
void StreamDataHandler(UNIT * unit, uint32_t preTrigger)
{
	...
	int32_t i, j;
	uint32_t sampleCount = config.sampleBuffer; /*  Make sure buffer size is large enough to copy data into on each iteration */
	...
	postTrigger = config.sampleNumber;
	autostop = TRUE;
	...
	sprintf_s(StreamFile,20,"f%.0f_a%.2f_%d.csv",freq[g_testCycle],amplitude[g_testCycle],g_singleTestCycle);
	/*StreamFile[7] = (g_testCycle>=10)?(g_testCycle/10+'0'):'0';
	StreamFile[8] = g_testCycle%10 + '0';
	StreamFile[10] = (g_singleTestCycle>=10)?(g_singleTestCycle/10+'0'):'0';
	StreamFile[11] = g_singleTestCycle%10 + '0';*/
	...
	if (fp != NULL)
	{
		...
		fprintf(fp, "frequency:,%.2f,",freq[g_testCycle]);
		fprintf(fp, "amplitude:,%.2f\n",amplitude[g_testCycle]);
		fprintf(fp, "Timestamp,");
		for (i = 0; i < unit->channelCount; i++)
		{
			if (unit->channelSettings[i].enabled) 
			{
				fprintf(fp,"Channel %C,",(int8_t)('A' + i));
			}
		}
		fprintf(fp, "\n");
		fprintf(fp, "(us),Force(N),Index(mV),Pwm(mV),Current(mA),Current(mA),Current(mA)\n");
	}
	...
	TriggerCallback(); //Trigger Arduino
	...
	while (!_kbhit() && !g_autoStopped)
	{
		...
		if (g_ready && g_sampleCount > 0) /* can be ready and have no data, if autoStop has fired */
		{
			...
			for (i = g_startIndex; i < (int32_t)(g_startIndex + g_sampleCount); i++)
			{
				...
				if(fp != NULL)
				{
					fprintf(fp, "%d,", timeDisplay++);
					for (j = 0; j < unit->channelCount; j++)
					{
						if (unit->channelSettings[j].enabled) 
						{
							int32_t temp = adc_to_mv(appBuffers[j * 2][i], unit->channelSettings[PS4000A_CHANNEL_A + j].range, unit);
							if(j==5||j==7)
							{
								fprintf(fp,"%5d,",temp/**80-200000*/);

							}
							else if(j==6)
							{
								fprintf(fp,"%5d,",temp/**40-100000*/);
							}
							else if(j == 1)
							{
								fprintf(fp,"%.4f,",((float)temp/1000.0*0.167+0.0067)/0.125);
							}
							else
							{
								 fprintf(fp,"%5d,",temp);
...
{
	int16_t triggerVoltage = mv_to_adc(2000, unit->channelSettings[PS4000A_CHANNEL_E].range, unit);
...
void SetTimebase(UNIT * unit)
{
	...
	while (ps4000aGetTimebase(unit->handle, timebase, BUFFER_SIZE, &timeInterval, &maxSamples, 0))
	{
		timebase++;  // Increase timebase if the one specified can't be used. 
	}

	printf("Timebase used %u = %dns sample interval\n", timebase, timeInterval);
}
...
void SetStreamingTrigger(UNIT * unit)
{
	int16_t triggerVoltage = mv_to_adc(1000,	unit->channelSettings[PS4000A_CHANNEL_D].range, unit); // ChannelInfo stores ADC counts
	ps4000aSetSimpleTrigger(unit->handle, 1, PS4000A_CHANNEL_D, triggerVoltage, PS4000A_RISING, 0, 0);
}
/****************************************************************************
...
void CollectStreamingTriggered(UNIT * unit)
{
	...
	struct tPS4000ACondition conditions [1] = {{PS4000A_CHANNEL_C,	PS4000A_CONDITION_TRUE }};
...
PICO_STATUS HandleDevice(UNIT * unit)
{
	...
	if (unit->openStatus == PICO_USB3_0_DEVICE_NON_USB3_0_PORT)
	{
	...
	for (i = 0; i < unit->channelCount; i++)
	{
		...
		unit->channelSettings[i].range = PS4000A_10V;
